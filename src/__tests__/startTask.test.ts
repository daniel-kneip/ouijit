...
describe('startTask', () => {
  ...
  test('concurrent startTask calls do not race to claim the same worktree directory', async () => {
    const { execFile } = await import('node:child_process');
    const fs = await import('node:fs/promises');

    // Stateful mock world: tracks which worktree directories exist. Two tasks
    // with the same name start at the same time with deploy-3 and deploy-4
    // already on disk, so both probes climb past them. Without the mutex they
    // would both target deploy-5 and one git worktree add would fail with
    // "already exists".
    const claimed = new Set(['deploy-3', 'deploy-4']);
    ...
    vi.mocked(execFile).mockImplementation(
      (
        ...
        if (Array.isArray(args) && args[0] === 'worktree' && args[1] === 'add') {
          const targetPath = args.find((a) => a.includes('/deploy-')) ?? '';
    ...
    await createTask(project, 3, 'Deploy', { status: 'todo' });
    await createTask(project, 5, 'Deploy', { status: 'todo' });
    ...
    // Task 3 enters first, probes past deploy-3/deploy-4, claims deploy-5.
    // Task 5 then enters, probes deploy-5 (now claimed) and lands at deploy-6.
    expect(baseOf(r3.worktreePath!)).toBe('deploy-5');
    expect(baseOf(r5.worktreePath!)).toBe('deploy-6');
  });
});
